...
def default_initial_preparation_common_to_all_cases_df_flows_not_baseline(
    df_od: pl.DataFrame,
    ...
    """
    df_od = join_Tij_Vodafone_with_distance_matrix(
        df_od=df_od,
    ...
    df_od_with_just_in_in_out_in_trips = pipeline_initial_df_flows_aggregation_on_dat_hour_user_weekday(
        df=df_od,
        tuple_filters=(
            pl.col(str_trip_type_od) != "out-out",
            pl.col(str_trip_type_od) != "in-out",
        ),
        message_filters=(
            f"{str_trip_type_od} != out-out",
            f"{str_trip_type_od} != in-out",
        ),
        list_columns_groupby=conditioning_2_columns_to_hold_when_aggregating[
            "day_hour_user_weekday"
        ],  # NOTE: T_{origin}_{destination}_{hour}_{user_profile}_{str_day}_{is_weekday}
...
def prepare_flow_dataframe_for_hierarchical_prcedure(
    ...
    columns_that_must_be_present_in_dataframes = (
        conditioning_2_columns_to_hold_when_aggregating[case]
    )
    for col in columns_that_must_be_present_in_dataframes:
        if col not in df_od_with_just_in_in_out_in_trips.columns:
            raise ValueError(
                f"Column {col} not present in df_od_with_just_in_in_out_in_trips"
            )
    ...
    Tij_dist_init = aggregate_flows(
        df=df_od_with_just_in_in_out_in_trips,
        list_columns_groupby=conditioning_2_columns_to_hold_when_aggregating[
            "day_hour_user_weekday"
...
def compute_filters_and_messages_for_case_analysis(
    ...
    dict_case_2_tuple_filters = {
        ...
        "user": (pl.col(str_origin_visitor_class_id_od) == int_user_profile,),
        "hour_weekday": (
            pl.col(col_str_is_week) == is_weekday,
            pl.col(str_departure_hour_od) == hour_id_of_interest,
        ),
        "hour": (pl.col(str_departure_hour_od) == hour_id_of_interest,),
        "weekday": (pl.col(col_str_is_week) == is_weekday,),
        "_": (),
    }
    ...
    dict_case_message = {
        ...
        "user": (f"{str_origin_visitor_class_id_od} == {user_profile}",),
        "hour_weekday": (
            f"{col_str_is_week} == {is_weekday}",
            f"{str_departure_hour_od} == {hour_id_of_interest}",
        ),
        "hour": (f"{str_departure_hour_od} == {hour_id_of_interest}",),
        "weekday": (f"{col_str_is_week} == {is_weekday}",),
        "_": (),
    }
...
def filter_flows_by_conditions_from_cases(
    ...
    (
        ...
        dict_case_message,
        dict_case_message_baseline,
    ) = compute_filters_and_messages_for_case_analysis(
    ...
    print(
        f"Filters to be applied to the flow dataframe for case {case_analysis}: {message_filters}"
    )
    print(
        f"Filters to be applied to the baseline flow dataframe for case {case_analysis}: {message_filters_baseline}"
    )
    # ✅ Use the helper instead of recursively calling this function
    Tij_dist = filter_flows_by_conditions(
        df=Tij_dist_init, tuple_filters=filters_flows, message=message_filters
    )
...
def define_columns_to_hold_and_merge_both_for_grid_and_flows_OD_analysis(
    columns_2_hold_geopandas_base: list,
    str_col_origin: str,
    ...
    dict_column_flows = set_dict_column_names_flows_OD_analysis(
    ...
    dict_column_grid = set_dict_column_names_grid_OD_analysis(
    ...
    extension_columns_2_hold = set_columns_to_hold_for_OD_analysis(
        dict_column_grid=dict_column_grid,
        str_day=str_day,
        time_interval=time_interval,
        user_profile=user_profile,
    ...
    print(
        f"Extension columns to hold for the grid analysis: {extension_columns_2_hold}"
    )
    (
