...
def initialize_geodataframe_polygons(
    ...
    cities_gdf = simple_join_cities_with_population(
        ...
        is_Vodafone_Trento_ZDT=True,
    )  # NOTE: here we join the cities with the population data from Istat
    ...
    cities_gdf = add_column_area_and_fraction(
        cities_gdf,
        str_col_comuni_name,
    ...
    cities_gdf = redistribute_population_by_fraction(
        cities_gdf,
        str_col_popolazione_totale=str_population_col_grid,
        str_col_fraction=str_col_fraction,
...
def get_informations_about_available_dates_from_files(
    bucket, s3, col_str_day_od, str_period_id_presenze, col_str_is_week
):
    """
    ...
    list_files_od, list_files_presenze, list_str_dates_yyyymm = (
        extract_filenames_and_date_from_bucket(bucket)
    )
    print("Extract days available for analysis flows...")
    list_all_avaliable_days_flows = (
        extract_all_days_available_analysis_flows_from_raw_dataset(
            list_files_od, col_str_day_od, str_period_id_presenze, col_str_is_week, s3
        )
    )  # NOTE: Needed to create dict_column_flows,grid for post-processing
    return (
        list_files_od,
        list_files_presenze,
...
def initialize_df_null_day_dataframe(
    cities_gdf,
    df_distance_matrix,
    s3,
    list_files_presenze,
    ...
    map_idx_cities_gdf_2_area_code = dict(
        zip(cities_gdf.index, cities_gdf[str_area_id_presenze])
    )  # create a map from the index of the cities gdf to the area code
    ...
    df_distance_matrix = add_column_area_code_OD_df_distance(
        df_distance_matrix,
    ...
    df_presenze_null_days = extract_presences_vodafone_from_bucket(
        s3, list_files_presenze, 2
    )  # NOTE: download the file from the bucket
    # NOTE: Extract OD
    df_od_null_days = extract_od_vodafone_from_bucket(s3, list_files_od, 2)
    df_od_null_days = add_column_is_week_and_str_day(
        df_od=df_od_null_days,
        str_period_id_presenze=str_period_id_presenze,
        col_str_day_od=col_str_day_od,
        col_str_is_week=col_str_is_week,
        is_null_day=True,
    ...
    df_od_null_days = join_Tij_Vodafone_with_distance_matrix(
        df_od=df_od_null_days,
...
def initialize_concatenated_od_dataframe(
    list_files_od,
    s3,
    str_period_id_presenze,
    col_str_day_od,
    ...
    """
    df_concat = concat_df_od_and_add_columns(
    ...
    df_od_with_just_in_in_out_in_trips = (
        default_initial_preparation_common_to_all_cases_df_flows_not_baseline(
            df_od=df_concat,
            df_distance_matrix=df_distance_matrix,
            str_origin_od=str_origin_od,
...
def level_aggregation_concat_od_dataframe_and_null_days(
    list_all_avaliable_days_flows,
    list_time_intervals,
    user_profile,
    UserProfiles,
    ...
    """
    (
        dict_column_flows_aggregation_weekday,
        dict_column_grid_aggregation_weekday,
        dict_output_hotspot_analysis_aggregation_weekday,
    ) = initialize_dicts_that_hold_grid_flows_columns_and_hotspot_analysis(
        list_all_avaliable_days_flows=list_all_avaliable_days_flows,
    ...
    print(
        "[DEBUG] Check the case pipeline and user profile: ",
        case_pipeline,
        user_profile,
        df_od_null_days.shape,
        df_od_with_just_in_in_out_in_trips.shape,
    )
    # NOTE: Aggregate the dataframe at the level you need
    Tij_dist_init, Tij_dist_baseline_init = (
        prepare_flow_dataframe_for_hierarchical_prcedure(
    ...
    return (
        ...
        dict_output_hotspot_analysis_aggregation_weekday,
        Tij_dist_init,
        Tij_dist_baseline_init,
    )


def main_diffusione_1_2_by_case_pipeline(
    global_Tij_holding_all_columns_flows,
    global_cities_gdf_holding_all_columns_flows,
    dict_column_flows_aggregation_weekday,
    dict_column_grid_aggregation_weekday,
    dict_output_hotspot_analysis_aggregation_weekday,  # Global variables that will contain the output
    Tij_dist_init,
    Tij_dist_baseline_init,  # Dataframes with the flows at the level of aggregation needed that will be used for in-out analysis
    ...
    time_interval,
    str_t,
    str_t1,  # Variables aggregation
    columns_2_hold_geopandas_base,
    str_col_origin,
    str_col_destination,
    str_grid_idx,  # Variables needed to be there as various columns for the analysis
    str_population_col_grid,
    str_col_comuni_name,
    str_hotspot_prefix,
    cities_gdf,
    ...
    for suffix_in, is_in_flows in case_2_is_in_flow.items():
        ...
        Tij_dist, Tij_dist_baseline = filter_flows_by_conditions_from_cases(
        ...
        Tij_dist = Tij_dist.with_columns(
            (pl.col("TRIPS")).alias(str_col_n_trips)
        )  # Divide by two the trips since we are considering both nationalities
        Tij_dist_baseline = Tij_dist_baseline.with_columns(
            pl.col("TRIPS").alias(str_col_n_trips_baseline)
        )  # NOTE: rename the column with the trips to the one needed for the analysis
        print(f"Compare Tij_dist to Tij_dist_baseline: {suffix_in}")
        Tij_dist = compute_difference_trips_col_day_baseline(
            Tij_dist=Tij_dist,
        ...
        (
            ...
            cities_gdf=geojson_input_hierarchy,
        ...
        hotspot_levels = get_values_from_case_pipeline_OD_analysis(
            ...
            is_in_flows=is_in_flows,
            suffix_in=suffix_in,
            name_dict="dict_output_hotspot_analysis",
            name_key="hotspot_levels",
            case_pipeline=case_pipeline,
        )
        ...
        if str_area_id_presenze in global_cities_gdf_holding_all_columns_flows.columns:
            global_cities_gdf_holding_all_columns_flows[str_area_id_presenze] = (
                global_cities_gdf_holding_all_columns_flows[
                    str_area_id_presenze
                ].astype(str)
            )
        ...
        grid_global = pl.from_pandas(
            global_cities_gdf_holding_all_columns_flows.copy().drop("geometry", axis=1)
        )
        # Memory management: Clear baseline analysis variables
        gc.collect()
        idx_case += 1
        print(
            f"Number columns global flows after join: ",
            len(global_Tij_holding_all_columns_flows.columns),
            f" cities: ",
            len(global_cities_gdf_holding_all_columns_flows.columns),
        )
    return (
        global_Tij_holding_all_columns_flows,
        grid_global,
...
def main_generate_flows_and_grids(
    ...
    (
        ...
        list_str_dates_yyyymm,
        list_all_avaliable_days_flows,
    ) = get_informations_about_available_dates_from_files(
        bucket=bucket,
        s3=s3,
        col_str_day_od=col_str_day_od,
        str_period_id_presenze=str_period_id_presenze,
    ...
    df_od_null_days, df_presenze_null_days, df_distance_matrix = (
        initialize_df_null_day_dataframe(
            cities_gdf=cities_gdf,
            df_distance_matrix=df_distance_matrix,
            s3=s3,
            list_files_presenze=list_files_presenze,
            list_files_od=list_files_od,
        )
    )
    ...
    columns_2_hold_geopandas_base = [
        ...
        str_grid_idx,
        "geometry",
    ]  # base columns to hold in the geopandas
    # NOTE: Initialize the concatenated OD dataframe
    df_od_with_just_in_in_out_in_trips = initialize_concatenated_od_dataframe(
        list_files_od=list_files_od,
        s3=s3,
        str_period_id_presenze=str_period_id_presenze,
        col_str_day_od=col_str_day_od,
    ...
    list_unique_days_od = (
        df_od_with_just_in_in_out_in_trips[col_str_day_od].unique().to_list()
    )
    ...
    case_pipelines_to_run = [
        cp
        for cp in conditioning_2_columns_to_hold_when_aggregating.keys()
        if cp in TARGET_CASE_PIPELINES
    ]
    missing = set(TARGET_CASE_PIPELINES) - set(case_pipelines_to_run)
    if missing:
        raise ValueError(
            f"Le chiavi richieste {missing} non sono in conditioning_2_columns_to_hold_when_aggregating"
        )
    ...
    for case_pipeline in case_pipelines_to_run:
        ...
        for combo in itertools.product(*conditioning_values):
            ...
            time_interval = loop_context.get("hour", [0, 0])
            ...
            int_hour_start_window_interest = time_interval[0]

            dir_components = [
                (
                    str(loop_context[var])
                    if var != "hour"
                    else f"{int(loop_context[var][0])}_{int(loop_context[var][1])}"
                )
            ...
            (
                ...
                Tij_dist_init,
                Tij_dist_baseline_init,
            ) = level_aggregation_concat_od_dataframe_and_null_days(
                list_all_avaliable_days_flows=list_all_avaliable_days_flows,
                list_time_intervals=list_time_intervals,
                user_profile=user_profile,
                UserProfiles=UserProfiles,
                ...
                case_pipeline=case_pipeline,
                df_od_with_just_in_in_out_in_trips=df_od_with_just_in_in_out_in_trips,
                df_od_null_days=df_od_null_days,
            )
