...
def get_loubar_threshold(remaining_grid, str_col_total_flows_grid: str) -> tuple:
    ...
    if hasattr(remaining_grid, "to_pandas"):  # Polars DataFrame
        ...
        actual_indices = np.arange(
            len(remaining_grid)
        )  # or use remaining_grid.get_column_index() if you have a specific index column
    ...
    chosen_flow_indices = [
        int(sorted_flow_indices[-i - 1]) for i in range(num_to_select)
    ]
...
def extract_hotspot_levels(
    ...
    if (
        len(flows_data) == 0
        or np.sum(np.isfinite(flows_data)) == 0
        or np.sum(flows_data) == 0
    ):
    ...
    while len(remaining) > 0 and len(levels) < max_levels:
        try:
            selected_idces, _, _ = get_loubar_threshold(
...
def get_critical_fluxes_per_hotspot_level(
    ...
    assert (
        str_hotspot_level_grid in grid.columns
    ), f"Column {str_col_total_flows_grid} not found in grid, look at the definition of the Mobility_Hierarchy class."
    ...
    for level, indices in hotspot_levels.items():
        ...
        for idx in indices:
            if is_in_flows:
                ...
                selected_flows = df_flows.filter(
                    pl.col(str_col_destination) == int(idx)
                )  # NOTE: choosing the flows that end in the hotspot chosen from in-fluxes
                ...
                ordered_local_indices_in_selected_flows, _, _ = get_loubar_threshold(
                    selected_flows, str_col_n_trips
                )
                ...
                grid_idces = [
                    int(x)
                    for x in selected_flows.loc[
                        ordered_local_indices_in_selected_flows, str_col_origin
                    ].to_numpy()
                ]

                if len(hotspot_2_origin_idx_2_crit_dest_idx[level][int(idx)]) == 0:
                    hotspot_2_origin_idx_2_crit_dest_idx[level][int(idx)] = list(
                        grid_idces
                    )
                    list_indices_all_fluxes_for_colormap.append(grid_idces)
                else:

                    hotspot_2_origin_idx_2_crit_dest_idx[level][int(idx)].extend(
                        grid_idces
                    )
            ...
            # NOTE: case critical index out flows
                ...
                grid_idces = [
                    int(x)
                    for x in selected_flows.loc[
                        ordered_local_indices_in_selected_flows, str_col_destination
                    ].to_numpy()
                ]
