    ...
    for obj in bucket.objects.filter(Prefix=f"{DATA_PREFIX}vodafone-aixpa"):
        ...
        if obj.key.endswith(".parquet"):
            if "od-mask" in obj.key and "01" not in obj.key:
                ...
                date_str = (
                    obj.key.split("/")[-1].split("_")[1].split(".")[0]
                )  # Extract date from the filename yyyymm
                ...
                date_str = (
                    obj.key.split("/")[-1].split("_")[1].split("-")[1].split(".")[0]
                )
...
def extract_presences_vodafone_from_bucket(
    s3, list_files_presenze: list[str], i: int
) -> pl.DataFrame:
    """
    Extract presences data from the Vodafone bucket.
    """
    print(
        f"Extracting presenze data from file {list_files_presenze[i]}"
    )  # print the file name
    object_presenze = s3.Object(
        "most-datalake", list_files_presenze[i]
...
def extract_od_vodafone_from_bucket(
    s3, list_files_od: list[str], i: int
) -> pl.DataFrame:
    """
...
def merge_flows_and_grid_with_global_to_obtain_unique_dfs(
    global_Tij_holding_all_columns_flows: pl.DataFrame | pd.DataFrame | None,
    ...
    if is_join_flows:
        if global_Tij_holding_all_columns_flows is None:
            ...
            print(
                f" - n flows before join: {len(global_Tij_holding_all_columns_flows)}"
            )
            global_Tij_holding_all_columns_flows = (
                global_Tij_holding_all_columns_flows.join(
                    flows_2_be_merged[columns_flows_2_be_merged_2_keep],
    ...
    if is_join_grid:
        ...
        print(
            f" - n grid before join: {len(global_cities_gdf_holding_all_columns_flows)}"
        )
        # Use merge instead of join for more explicit control over join keys
        global_cities_gdf_holding_all_columns_flows = (
            global_cities_gdf_holding_all_columns_flows.merge(
                ...
                suffixes=(
                    "",
                    "_right",
                ),  # Add suffix to handle potential duplicate columns
            )
        )
        # Drop duplicate columns that may have been added from the merge
        cols_to_drop = [
            col
            for col in global_cities_gdf_holding_all_columns_flows.columns
            if col.endswith("_right")
        ]
        if cols_to_drop:
            global_cities_gdf_holding_all_columns_flows = (
                global_cities_gdf_holding_all_columns_flows.drop(columns=cols_to_drop)
            )
        print(
            f" - n grid after join: {len(global_cities_gdf_holding_all_columns_flows)}"
        )
    return (
        global_Tij_holding_all_columns_flows,
        global_cities_gdf_holding_all_columns_flows,
