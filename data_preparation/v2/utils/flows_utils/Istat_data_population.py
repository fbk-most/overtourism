...
def simple_join_cities_with_population(
    cities_gdf,
    population_df,
    ...
    population_df["Comune_normalized"] = (
        population_df[str_col_comuni_istat].str.lower().str.strip()
    )
    ...
    for idx, row in result_gdf.iterrows():
        ...
        for component in city_components:
            matches = population_df[population_df["Comune_normalized"] == component]
            if not matches.empty:
                matched_population += matches[str_col_popolazione_totale].sum()
                matched_comuni.extend(matches[str_col_comuni_istat].tolist())

        city_to_population[city_name] = (
            matched_population if matched_population > 0 else None
        )

        if matched_comuni:
            print(f"'{city_name}' -> {matched_comuni} = {matched_population}")

    result_gdf[str_col_popolazione_totale] = result_gdf[str_col_city_name].map(
        city_to_population
    )
...
def _normalize_trento_zdt(city_name):
    ...
    trento_patterns = [r"Trento_ZDT_", r"Trento_ZDT"]
...
class Istat_population_data:
    ...
    def load_data(self, local=False):
        ...
        if local:
            ...
            self.df_istat = pl.DataFrame(
                pd.read_csv(get_s3(Path(self.path).name), sep=";")
            )
