...
def main_generate_flows_and_grids(str_name_project = str_name_project, str_dir_data_path = str_dir_data_path, str_dir_output_path = str_dir_output_path, complete_path_Istat_population = complete_path_Istat_population,
    ...
    case_pipelines_to_run = [
        cp for cp in conditioning_2_columns_to_hold_when_aggregating.keys()
        if cp in TARGET_CASE_PIPELINES
    ]
    missing = set(TARGET_CASE_PIPELINES) - set(case_pipelines_to_run)
    if missing:
        raise ValueError(f"Le chiavi richieste {missing} non sono in conditioning_2_columns_to_hold_when_aggregating")
    ...
    for case_pipeline in case_pipelines_to_run:
        ...
        for combo in itertools.product(*conditioning_values):
            ...
            dir_components = [
                str(loop_context[var]) if var != "hour" else f"{int(loop_context[var][0])}_{int(loop_context[var][1])}"
                for var in pipeline_vars if var in loop_context
            ]
            ...
            dict_column_flows_aggregation_weekday, dict_column_grid_aggregation_weekday, \
                dict_output_hotspot_analysis_aggregation_weekday, Tij_dist_init, Tij_dist_baseline_init = \
                level_aggregation_concat_od_dataframe_and_null_days(
                    list_all_avaliable_days_flows=list_all_avaliable_days_flows,
                    list_time_intervals=list_time_intervals, user_profile=user_profile,
                    UserProfiles=UserProfiles, week_days=week_days, case_2_is_in_flow=case_2_is_in_flow,
                    case_pipeline=case_pipeline, df_od_with_just_in_in_out_in_trips=df_od_with_just_in_in_out_in_trips,
            ...
            global_Tij_holding_all_columns_flows, grid_global, global_cities_gdf_holding_all_columns_flows = main_diffusione_1_2_by_case_pipeline(
                    ...
                    Tij_dist_init=Tij_dist_init, Tij_dist_baseline_init=Tij_dist_baseline_init,
                    case_2_is_in_flow=case_2_is_in_flow, case_pipeline=case_pipeline, user_profile=user_profile,
                    is_weekday=is_weekday, str_day=str_day, int_hour_start_window_interest=int_hour_start_window_interest,
                    time_interval=time_interval, str_t=str_t, str_t1=str_t1,
                    columns_2_hold_geopandas_base=columns_2_hold_geopandas_base, str_col_origin=str_col_origin,
                    str_col_destination=str_col_destination, str_grid_idx=str_grid_idx,
                    str_population_col_grid=str_population_col_grid, str_col_comuni_name=str_col_comuni_name,
                    str_hotspot_prefix=str_hotspot_prefix, str_centroid_lat=str_centroid_lat,
