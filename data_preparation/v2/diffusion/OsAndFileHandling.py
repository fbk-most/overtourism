...
def extract_presences_vodafone_from_bucket(s3,list_files_presenze:list[str], i:int) -> pl.DataFrame:
    ...
    object_presenze = s3.Object('most-datalake', list_files_presenze[i])                                                                                                      # NOTE: download the file from the bucket
    buffer=io.BytesIO()
...
def extract_od_vodafone_from_bucket(s3,list_files_od:list[str], i:int) -> pl.DataFrame:
    ...
    object_od = s3.Object('most-datalake', list_files_od[i])                                                                                                                  # NOTE: download the file from the bucket
    buffer=io.BytesIO()
