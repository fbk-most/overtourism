...
"""Streamlit dashboard for Molveno, driven entirely over HTTP by `overtourism.api`.

Unlike `molveno_dashboard.py`, this process never imports `MolvenoBackend` or
...
def _get_adapter() -> HttpOvertourismAdapter:
    return HttpOvertourismAdapter(
        model_key="molveno",
        title="Overtourism Digital Twin — Molveno [API]",
        base_url=_BASE_URL,
    )
